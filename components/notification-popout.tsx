...
export function NotificationPopout({ open, onClose, onSelectTask }: NotificationPopoutProps) {
  ...
  const markAllNotificationsAsRead = useWorkflowStore((state) => state.markAllNotificationsAsRead)
  ...
  const filteredNotifications = useMemo(() => {
    const all = workflowSelectors.getFilteredNotifications(notifications, currentUser, tasks)
  ...
  return (
    <>
      ...
      <div className="absolute top-16 right-0 w-[380px] bg-surface rounded-2xl shadow-2xl border border-outline-variant z-[70] overflow-hidden flex flex-col animate-in slide-in-from-top-4 duration-200">
        ...
        <div className="p-3 bg-surface-container-low border-t border-outline-variant flex justify-between px-4">
          <button 
          ...
          <button 
            className="text-[11px] font-bold text-on-surface-variant hover:underline uppercase tracking-wider"
            onClick={onClose}
          >
            Cerrar Panel
          </button>
